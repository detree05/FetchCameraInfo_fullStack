  ...
  const InitRequest = () => {
    axios.post('http://172.16.0.57:8888/getChannel', CreateQuery()).then(res => {
      ...
      for (var i = 0; i < resData.length; i++) {
        ...
        `
        if (resData[i]["ChannelConfigLink"]) {
          resDataArray[i] += ` | @channel_config: ${resData[i]["ChannelConfigLink"]}@`
        }
        if (resData[i]["ChannelStatusLink"]) {
          resDataArray[i] += ` | @channel_status: ${resData[i]["ChannelStatusLink"]}@`
        }
        if (resData[i]["CameraAvailability"]) {
          resDataArray[i] += ` | @camera_availability: ${resData[i]["CameraAvailability"]}@`
        }
      }
